...
*  The gun of each state is from [ThoughtCo](https://www.thoughtco.com/gun-owners-percentage-of-state-populations-3325153)
containing information on number of guns per capita and total guns registered of each state in 2017.
...
We directly download the csv file of world suicide data [here]( https://www.kaggle.com/russellyates88/suicide-rates-overview-1985-to-2016). Then calculate for suicide rate per 100k using suicide number and total population. Next for different purposes, we `group_by` different variables such as year, country and gender to get the rates in different groups. 
...
We made a line plot to show the change of total suicide rate by year in US. We can see a stable increase by year and the truth that there are much more cases of suicide in male than in female.
```{r message = FALSE}
...
We also made boxplots to show the distributions of suicides cases in states and population composition in states.
```{r message=FALSE, warning=FALSE}
